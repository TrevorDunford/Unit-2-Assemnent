...
let canWeDeliver = (zipcode) => {
 let trueOrfasle =   deliveryAreaZipCodes.includes(zipcode)
    if (trueOrfasle = false) {
...
let canWeDeliverTwo = (zipcode) => {deliveryAreaZipCodes.filter(zipcode)
...
let newdeal = deals.replace(15, 10)
console.log(newdeal)
...
let newerdeal= deals.replace(March, April)